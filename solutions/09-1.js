...
function doInstruction(pointer) {
  ...
  var start = pointer + 1;
  var mid = start + (opcode === OUTPUT || opcode === OFFSET ? 1 : 2);
  if (opcode === INPUT) mid = start;
  var end = mid + 1;
  if (opcode === OUTPUT || opcode === OFFSET) end = mid;
...
function doCommand(opcode, modes, inputs, outputPos, currPointer) {
  function getValue(input, mode) {
    ...
    if (mode === RELATIVE) return list[relativePos(input, mode)];
    return list[input];
  }
  function relativePos(pos, mode) {
    if (mode === RELATIVE) return pos + relativeBase;
    return pos;
  }
  ...
  if (opcode === ADD) {
    ...
    list[relativePos(outputPos, modes[2])] = value;
  }
  if (opcode === MULTIPLY) {
  ...
  if (opcode === INPUT) {
    list[relativePos(outputPos, modes[0])] = input;
  }
  ...
  if (opcode === LESSTHAN) {
    var value1 = getValue(inputs[0], modes[0]);
    var value2 = getValue(inputs[1], modes[1]);
    list[relativePos(outputPos, modes[2])] = value1 < value2 ? 1 : 0;
  }
  if (opcode === EQUALS) {
    var value1 = getValue(inputs[0], modes[0]);
    var value2 = getValue(inputs[1], modes[1]);
  ...
  if (opcode === OFFSET) {
    var baseOffset = getValue(inputs[0], modes[0]);
    relativeBase += baseOffset;
  }
...
var pointer = 0;
//var i=0;
//if (i < 10) console.log(list, relativeBase, pointer);
try {
  while (true) {
    try {
