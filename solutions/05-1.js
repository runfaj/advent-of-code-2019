...
function doInstruction(pointer) {
  ...
  if ([ADD, MULTIPLY, INPUT, OUTPUT, EXIT].indexOf(opcode) === -1) throw "invalid opcode";
  ...
  //console.log(codes, inputs, outputPos, modes);
  doCommand(opcode, modes, inputs, outputPos);
  return end;
}

function doCommand(opcode, modes, inputs, outputPos) {
  var inputValues = inputs.map((input,i) => {
    if (modes[i] === IMMEDIATE) return input;
    return list[input];
  });
  ...
  if (opcode === OUTPUT) {
    var value = list[inputs[0]];
    if (modes[0] === IMMEDIATE) value = inputs[0];
